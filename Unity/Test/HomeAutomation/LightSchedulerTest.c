...
TEST(LightScheduler, RejectTooManyEvents)
{
	...
	TEST_ASSERT_EQUAL(LS_TOO_MANY_EVENTS, LightScheduler_scheduleTurnOn(6, MONDAY, 600 + i));
}
...
TEST(LightScheduler, RemoveRecyclesScheduleSlot)
{
	...
	TEST_ASSERT_EQUAL(LS_OK, LightScheduler_scheduleTurnOn(13, MONDAY, 1000));
}
...
{
	TEST_ASSERT_EQUAL(LS_OK, LightScheduler_scheduleTurnOn(0, MONDAY, 1000));
	TEST_ASSERT_EQUAL(LS_OK, LightScheduler_scheduleTurnOn(15, MONDAY, 1000));
	TEST_ASSERT_EQUAL(LS_OK, LightScheduler_scheduleTurnOn(31, MONDAY, 1000));
}

TEST(LightScheduler, RejectsInvalidLightIds)
{
	TEST_ASSERT_EQUAL(LS_ID_OUT_OF_BOUNDS, LightScheduler_scheduleTurnOn(-1, MONDAY, 1000));
	TEST_ASSERT_EQUAL(LS_ID_OUT_OF_BOUNDS, LightScheduler_scheduleTurnOn(32, MONDAY, 1000));
}
...
{
	LightScheduler_create();
	TEST_ASSERT_TRUE((void*)LightScheduler_wakeup == (void*)FakeTimeService_getAlarmCallback());
...
TEST(LightSchedulerInitAndCleanup, DestroyCancelsOneMinuteAlarm)
{
	LightScheduler_create();
	LightScheduler_destroy();
	TEST_ASSERT_TRUE(NULL == (void*)FakeTimeService_getAlarmCallback());
}
...
TEST_GROUP_RUNNER(LightScheduler)
{
	...
	RUN_TEST_CASE(LightScheduler, NoSchedulerNothingHappens);
	...
	RUN_TEST_CASE(LightScheduler, RejectTooManyEvents);
